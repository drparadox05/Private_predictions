...
run_cre_workflow() {
  ...
  cre_output=$(CRE_ETH_PRIVATE_KEY="$CRE_ETH_PRIVATE_KEY" cre workflow simulate "$workflow_dir" --project-root . --target "$CRE_TARGET" --non-interactive --trigger-index 0 --evm-tx-hash "$trigger_tx_hash" --evm-event-index 0 --broadcast)
  popd >/dev/null
  printf '%s\n' "$cre_output"
  local cre_tx_hash
...
process_once() {
  ...
  perform_output=$(cast send "$MARKET_ADDRESS" 'performUpkeep(bytes)' "$perform_data" --rpc-url "$RPC_URL" --private-key "$AUTOMATION_PRIVATE_KEY" --json)
